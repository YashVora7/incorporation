...
<?php
 ...
 $compliance_id = isset($_GET['compliance_id'])? $_GET['compliance_id']:' ';
...
$sql_sign_cddkyc_compliance = mysqli_query($link, "
    SELECT * 
    FROM compliance_sign_cddkyc 
    WHERE company_id = '$company_id' 
      AND officer_id = '$get_officer_id' 
      AND compliance_id = '$compliance_id'
");

if (!$sql_sign_cddkyc_compliance) {
    die('Query Failed: ' . mysqli_error($link));
}

$result_sign_cddkyc_compliance = mysqli_fetch_assoc($sql_sign_cddkyc_compliance);

if ($result_sign_cddkyc_compliance) {
    // Construct the file path
    $uploadDir = __DIR__ . '../../../internal-staff/uploads/cddkyc_form_sign_by_compliance/'.$result_sign_cddkyc_compliance['compliance_sign_cddkyc_doc'];
...
$envelopeId = sendEnvelope($accessToken, $accountId, $documentBase64, $result, $brandId,$result_user_role);
...
function getEnvelopeRecipients($accessToken, $accountId, $envelopeId) {
 ...
 function sendEnvelope($accessToken, $accountId, $documentBase64, $result, $brandId,$result_user_role) {
    $apiBase = 'https://demo.docusign.net/restapi';
    ...
    $responseData = json_decode($response, true);
     ...
     <script>
            swal.fire({
                ...
                if (isConfirm.value) {
                    window.location = '../new_companies/cddkyc_verify.php';
                }
            });
        </script>
        <?php
...
function envelopeView($accessToken, $accountId, $envelopeId, $baseUrl, $officer_id, $result) {
    ...
    $data = [
        "returnUrl" => $baseUrl . "super_admin/docsign_api/cddkyc_save.php?en_id=" . $envelopeId."&director_id=" . $officer_id."&company_id=" . $result['cr_id'],
