...
if (isset($_GET['en_id'])) {
    ...
    if ($response === false) {
        ...
        $query = "INSERT INTO compliance_sign_cddkyc (company_id, officer_id, compliance_id, compliance_sign_cddkyc, compliance_sign_cddkyc_doc)
                  VALUES ('$company_id', '$directorId', '$user_id', '$signFlag', '$pdfName')";

        if (mysqli_query($link, $query)) {
            $_SESSION['document_sign_successfully'] = "Document Sign Successfully";
            header("Location: " . $baseUrl . "internal-staff/company-dashboard/cddkyc.php?company_id=" . $company_id);
